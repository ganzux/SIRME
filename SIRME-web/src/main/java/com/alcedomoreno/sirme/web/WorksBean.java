...
import com.alcedomoreno.sirme.business.validator.Validator;
...
public class WorksBean extends ManagedBean {
	...
	public void reloadAvaiableReports(){
		...
		for ( Report all:applicationBean.getReportsCollection() ){
			...
			// Si el Report esta incluído en el trabajo activo, no se incluye
					...
					for ( Reply reply:replieGroup.getReplies() ){
						...
						if ( rpl != null && !rpl.trim().isEmpty() ){
							try {
								...
								reply.setReply( rpl );
							} catch (com.alcedomoreno.sirme.business.validator.ValidationException e) {
